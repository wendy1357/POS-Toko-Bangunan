  ...
  <div v-if="show" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
    <div class="relative mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
      <div class="mt-3 text-center">
        <h3 class="text-lg leading-6 font-medium text-gray-900">{{ customer?.id ? "Edit" : "Tambah" }} Pelanggan</h3>
        <div class="mt-2 px-7 py-3">
          <form @submit.prevent="save" class="space-y-4 text-left">
            <div>
              <label for="nama" class="block text-sm font-medium text-gray-700">Nama</label>
              <input v-model="form.nama" type="text" id="nama" required class="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md" />
            </div>
            <div>
              <label for="no_hp" class="block text-sm font-medium text-gray-700">No. HP</label>
              <input v-model="form.no_hp" type="text" id="no_hp" class="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md" />
            </div>
            <div>
              <label for="alamat" class="block text-sm font-medium text-gray-700">Alamat</label>
              <textarea v-model="form.alamat" id="alamat" rows="3" class="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md"></textarea>
            </div>
